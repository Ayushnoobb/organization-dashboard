...
const BlockButton : React.FC<{id : string , disabled : boolean}> = ({id , disabled}) => {
  const dispatch = useAppDispatch()
  const [isLoading, setIsLoading] = useState(false);  
  const [toastDisplayed, setToastDisplayed] = useState(false);

  const { isFulfilled : blockFulfill , isPending : blockPending , isRejected : blockRejected } = useSelector((state : RootStore) => state.block_user)
  ...
  useEffect(() => {
    ...
    if (blockRejected && isLoading && !toastDisplayed) {
      setIsLoading(false); 
      setToastDisplayed(true); 
      errorToast('Error blocking user!');
      dispatch(resetState());
    }
  ...
  return (
    <button className={`bttn edit-btn ${isLoading ? 'loading' : ''}`} onClick={handleClick} disabled={isLoading || disabled}>
